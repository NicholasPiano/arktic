...
class Transcription(models.Model):
  ...
  def line(self):
    ...
    path = './' + path[path.index('2014'):]
    return '%s|%s|%s|%s|%s|%d\n' % (path, self.grammar.csv_file.file_nameg, self.confidence, self.revisions.latest().utterance, self.value, int(1000*float(self.confidence_value)) if self.confidence_value else 0)
